...
// initialize training constants, e.g., learning rate and regularization coefficient
...
// compute accuracy
	double acc = (1.0 * (tp + tn)) / (1.0 * (tp + tn + fp + fn));
...
//	data::Save("test_ver.csv", test_dat_reg);
		data::Save("traintar_iris.csv", resp_train_reg);
		data::Save("testtr_iris.csv", resp_test_reg);
		data::Save("traindat_iris.csv", train_dat_reg);
		data::Save("test_iris.csv", test_dat_reg);
...
// train the classification model
	...
	for (int i = 0; i <= test_row_reg; i++) {
		for (int j = 0; j < 3; j++) {
			cout << decrypt_frac(weights_reg[i][j], decryptor, frencoder) << endl;
		}
...
//make multiclass predictions
	...
	predictions_reg = lr.before_pred_mul(encoded_test_reg, weights_reg,
...
// array to store result of prediction for each data point and classifier
	double** act_pred = new double*[test_col_reg];
	act_pred = lr.predict_multiclass(k, test_col_reg, pred_dec);
...
//	data::Load("test_reg_ridge.csv", test_dat);
//	data::Load("train_reg_ridge.csv", train_dat);
//	data::Load("test_reg_tr_ridge.csv", resp_test);
//	data::Load("train_reg_tr_ridge.csv", resp_train);
...
//	data::Save("aba_test_sim.csv", test_dat);
//	data::Save("aba_train_si.csv", train_dat);
//	data::Save("aba_test_resp_sim.csv", resp_test);
//	data::Save("aba_train_resp_sim.csv", resp_train);
...
// initialize constants for training, e.g. learning rate and regularization coefficient
...
// train the regression model
	...
	weights = linreg.train(alpha, 5, train_col, encoded_train, train_resp,theta, train_row, text, evaluate, ridge, lambda_div);
	end = chrono::steady_clock::now();
	diffr = end - start;
	cout << "train " << chrono::duration<double, ratio<60>>(diffr).count()
			<< " min" << endl;
...
// compute Mean Squared Error
	...
	cout << " Mean squared error " << MSE / (100.0) << endl;
}
