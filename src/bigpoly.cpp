...
BigPoly::BigPoly() :
		value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), is_alias_(
				false) {
}
...
int BigPoly::significant_coeff_count() const {
	...
	int coeff_uint64_count = divide_round_up(coeff_bit_count_, bits_per_uint64);
	return get_significant_coeff_count_poly(value_, coeff_count_,
			coeff_uint64_count);
}
...
void BigPoly::set_zero() {
	if (coeff_count_ > 0 && coeff_bit_count_ > 0) {
		int coeff_uint64_count = divide_round_up(coeff_bit_count_,
				bits_per_uint64);
...
void BigPoly::set_zero(int start_coeff, int length) {
	...
	if (length < 0 || start_coeff + length > coeff_count_) {
		throw out_of_range(
				"length must be non-negative and start_coeff + length must be within [0, coefficient count]");
	}
	if (length > 0 && coeff_bit_count_ > 0) {
		int coeff_uint64_count = divide_round_up(coeff_bit_count_,
				bits_per_uint64);
		uint64_t *coeff = get_poly_coeff(value_, start_coeff,
				coeff_uint64_count);
...
void BigPoly::resize(int coeff_count, int coeff_bit_count) {
	...
	int old_coeff_uint64_count = divide_round_up(coeff_bit_count_,
			bits_per_uint64);
	int new_coeff_uint64_count = divide_round_up(coeff_bit_count,
			bits_per_uint64);
	if (old_coeff_uint64_count == new_coeff_uint64_count
			&& coeff_count == coeff_count_) {
		uint64_t *coeff = value_;
		for (int i = 0; i < coeff_count; ++i) {
			filter_highbits_uint(coeff, new_coeff_uint64_count,
					coeff_bit_count);
	...
	// Allocate new space.
	uint64_t *new_value =
			coeff_count > 0 && new_coeff_uint64_count > 0 ?
					new uint64_t[coeff_count * new_coeff_uint64_count] :
					nullptr;
	...
	if (coeff_count > 0 && new_coeff_uint64_count > 0) {
		...
		for (int i = 0; i < coeff_count; ++i) {
			if (i < coeff_count_) {
				set_uint_uint(from_coeff, old_coeff_uint64_count,
						new_coeff_uint64_count, to_coeff);
				filter_highbits_uint(to_coeff, new_coeff_uint64_count,
						coeff_bit_count);
			} else {
...
void BigPoly::alias(int coeff_count, int coeff_bit_count,
		std::uint64_t *value) {
	if (coeff_count < 0) {
		throw invalid_argument("coeff_count must be non-negative");
	}
	...
	if (value == nullptr && (coeff_count > 0 || coeff_bit_count > 0)) {
		throw invalid_argument(
				"value must be non-null for non-zero coefficient and bit counts");
	}
	...
	// Create coefficients.
	if (coeff_count > 0) {
		int coeff_uint64_count = divide_round_up(coeff_bit_count,
				bits_per_uint64);
		coeffs_ = new BigUInt[coeff_count];
...
BigPoly &BigPoly::operator =(const BigPoly& assign) {
	...
	if (coeff_count_ < assign_sig_coeff_count
			|| coeff_bit_count_ < assign_max_coeff_bit_count) {
...
BigPoly &BigPoly::operator =(const string &hex_poly) {
	...
	while (pos < length) {
		...
		int coeff_bit_count = get_hex_string_bit_count(hex_poly_ptr + pos,
				coeff_length);
	...
	while (pos < length) {
		...
		for (int zero_power = last_power - 1; zero_power > power;
				--zero_power) {
		...
		hex_string_to_uint64(coeff_start, coeff_length, coeff_uint64_count,
				coeff);
		last_power = power;
	}
...
void BigPoly::save(ostream &stream) const {
	...
	stream.write(reinterpret_cast<const char*>(&coeff_count32),
			sizeof(int32_t));
	int32_t coeff_bit_count32 = static_cast<int32_t>(coeff_bit_count_);
	stream.write(reinterpret_cast<const char*>(&coeff_bit_count32),
			sizeof(int32_t));
	int coeff_uint64_count = divide_round_up(coeff_bit_count_, bits_per_uint64);
	stream.write(reinterpret_cast<const char*>(value_),
...
void BigPoly::load(istream &stream) {
	...
	stream.read(reinterpret_cast<char*>(&read_coeff_bit_count),
			sizeof(int32_t));
	if (read_coeff_count > coeff_count_
			|| read_coeff_bit_count > coeff_bit_count_) {
		// Size is too large to currently fit, so resize.
		resize(max(read_coeff_count, coeff_count_),
				max(read_coeff_bit_count, coeff_bit_count_));
	}
	int read_coeff_uint64_count = divide_round_up(read_coeff_bit_count,
			bits_per_uint64);
	int coeff_uint64_count = divide_round_up(coeff_bit_count_, bits_per_uint64);
	if (read_coeff_uint64_count == coeff_uint64_count) {
		stream.read(reinterpret_cast<char*>(value_),
		...
		for (int i = 0; i < read_coeff_count; ++i) {
			stream.read(reinterpret_cast<char*>(coeff_ptr),
					read_coeff_uint64_count * bytes_per_uint64);
	...
	if (coeff_count_ > read_coeff_count) {
		set_zero_poly(coeff_count_ - read_coeff_count, coeff_uint64_count,
				value_ + read_coeff_count * coeff_uint64_count);
	}
}
