...
seal::Ciphertext** gradient_descent(int iterations, int col, int row,
		seal::Ciphertext** data, seal::Ciphertext** theta,
		seal::Ciphertext* target, seal::Evaluator evaluate, seal::Plaintext fr,
		seal::Plaintext a0, seal::Plaintext a1, seal::Plaintext a2,
		seal::Plaintext minus, seal::Plaintext tr, seal::Plaintext alpha) {
...
// y_i - log_h(x_i)
		...
		J[i] = compute_cost(col, row, theta, data, target, fr, evaluate, a0, a1,
				a2, minus, tr);
	}
	return thet;
}
...
// term1 = (theta*x) * a_1
	seal::Ciphertext term1 = seal::Ciphertext(
			evaluate.multiply_plain(tmp.operator const seal::BigPolyArray &(),
					a1));
//(theta*x)²
...
//term2 = (theta*x)² * a_2
	seal::Ciphertext term2 = seal::Ciphertext(
			evaluate.relinearize(
					evaluate.multiply_plain(
							sq_tmp.operator const seal::BigPolyArray &(), a2)));
// a_0 + term 2
...
seal::Ciphertext log_h_multiclass(bool neg, seal::Ciphertext* data,
		seal::Ciphertext** theta, seal::Plaintext a0, seal::Plaintext a1,
		seal::Plaintext a2, seal::Evaluator evaluate, int row, int col, int l) {
	seal::Ciphertext res;
	vector<seal::Ciphertext> ve;
...
seal::Ciphertext compute_helper(int col, int row, seal::Ciphertext** theta,
		seal::Ciphertext** data, seal::Ciphertext* target, seal::Plaintext fr,
		seal::Evaluator evaluate, seal::Plaintext a0, seal::Plaintext a1,
		seal::Plaintext a2, seal::Plaintext minus_one) {
...
// multiply with fr=1/col
	res = seal::Ciphertext(
			evaluate.multiply_plain(re.operator const seal::BigPolyArray &(),
...
seal::Ciphertext compute_cost(int col, int row, seal::Ciphertext** theta,
		seal::Ciphertext** data, seal::Ciphertext* target, seal::Plaintext fr,
		seal::Evaluator evaluate, seal::Plaintext a0, seal::Plaintext a1,
		seal::Plaintext a2, seal::Plaintext minus_one, seal::Plaintext tr) {
	seal::Ciphertext sum;
	seal::Ciphertext res;
	vector<seal::Ciphertext> ve;
	for (int i = 1; i <= row; i++) {
	...
	seal::Ciphertext tl2 = seal::Ciphertext(
			evaluate.add(tl.operator const seal::BigPolyArray &(),
					sum.operator const seal::BigPolyArray &()));
	res = seal::Ciphertext(
			evaluate.multiply_plain(tl2.operator const seal::BigPolyArray &(),
